      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {projects.map((project) => (
            <article
              key={project.title}
              className="flex flex-col rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur p-6 shadow-lg transition-transform duration-200 hover:-translate-y-1 hover:shadow-xl"
            >
              <div className="mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    {project.repo && (
                      <a
                        ...
                        className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-700 text-slate-300 transition-colors duration-200 hover:border-cyan-400 hover:text-cyan-300"
                ...
                {project.status && (
                  <span className="mt-3 inline-flex items-center gap-1 rounded-full bg-slate-800 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-cyan-300">
              ...
              <ul className="space-y-2 text-sm text-slate-200 mb-4">
                {project.highlights.map((highlight) => (
                  <li key={highlight} className="flex gap-2">
                    <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-cyan-400"></span>
                    <span>{highlight}</span>
                  </li>
                ))}
              </ul>
